...
trait getProduct {
    ...
    function getProductsWithPrice($keyword){
        if(Auth::guard('user')->check()){
            $user = Auth::guard('user')->user();
            $user->level = $user->user_level()->first();
            $products = Product::where('title', 'LIKE', '%' . $keyword . '%')->orWhere('sku', 'LIKE', '%' . $keyword . '%')->with(['product_type', 'product_price', 'product_detail', 'product_variations', 'product_brand', 'product_category',
            'product_price.product_price_details' => function($query) use ($user) {
                $query->where('user_price_id', '=', $user->level->user_price_id); 
