    @foreach ($carts as $cart_store)
        <div class="store-block" id="store{{ $cart_store->store->id }}" >
            <div class="store-title d-flex justify-content-between">
                ...
                <label for="receiverstore{{ $cart_store->store->id }}"><input
                        class="receiverstore" type="checkbox"
                        id="receiverstore{{ $cart_store->store->id }}"
                        name="receiverstore{{ $cart_store->store->id }}"
                        value="{{ $cart_store->store->id }}"
                        data-storeid="{{ $cart_store->store->id }}"
                ...
                <div class="row">
                    <div class="col-md-8 col-lg-8 col-xs-12 col-12">
                        <div class="d-flex justify-content-between">
                            <ul>
                                <li>Dịch vụ vận chuyển: {{$cart_store->transpot_service->title}}</li>
                                <li>@include('checkout.include.option_type_ship')</li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-md-4 col-lg-4 col-xs-12 col-12">
                        <div class="d-flex justify-content-end">
                            <span>Tổng tiền: </span>
                            <b
                                class="text-danger ml-1 total-cost" data-total_product="{{$cart_store->cart_item->sum('total_p')}}">{{ formatCurrency($cart_store->cart_item->sum('total_p') + $cart_store->transpot_price_default) }}</b>
                        </div>
                    </div>
                </div>
            </div>
