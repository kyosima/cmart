    ...
    <tr class="replaywith-{{ $role->id }}">
        ...
        <td>
            <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                <button type="button" class="btn btn-warning ajax-edit ajax-get-roles" data-id="{{ $role->id }}"
                    data-name="{{ $role->name }}" data-url="{{ route('roles.edit', $role->id) }}"><i
                        class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
                <button type="button" class="btn btn-danger ajax-delete"
                    data-url="{{ route('roles.destroy', $role->id) }}"><i class="fa fa-trash"></i></button>
            </div>
        </td>
    </tr>
@endif
...
@if ($type == 'permission')
    <tr class="replaywith-{{ $permission->id }}">
        ...
        <td>
            <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                <button type="button" class="btn btn-warning ajax-edit" data-id="{{ $permission->id }}"
                    data-name="{{ $permission->name }}"><i class="fa fa-pencil-square-o"
                        aria-hidden="true"></i></button>
                <button type="button" class="btn btn-danger ajax-delete"
                    data-url="{{ route('permissions.destroy', $permission->id) }}"><i
...
@if ($type == 'Permissions Role')
    ...
    @foreach ($permissions as $value)
        <option value="{{ $value->id }}" {{ checkRoleHasPermissions($role, $value->name) }}>{{ $value->name }}
        </option>
    @endforeach
@endif
...
@if ($type == 'adminPermission')
    <option value="">Vui lòng chon</option>
    @foreach ($permissions as $value)
        <option value="{{ $value }}" {{ checkAdminHasPermissionTo($admin, $value) }}>{{ $value }}
        </option>
    @endforeach
@endif
...
@if ($type == 'admin')
    <tr class="replaywith-{{ $admin->id }}">
        ...
        <td>
            <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                <button type="button" class="btn btn-warning ajax-edit ajax-get-admin" data-id="{{ $admin->id }}"
                    data-fullname="{{ $admin->fullname }}" data-DVCQ="{{ $admin->DVCQ }}"
                    data-url="{{ route('manager-admin.edit', $admin->id) }}"><i class="fa fa-pencil-square-o"
                        aria-hidden="true"></i></button>
                <button type="button" class="btn btn-danger ajax-delete"
                    data-url="{{ route('manager-admin.destroy', $admin->id) }}"><i class="fa fa-trash"></i></button>
            </div>
        </td>
    </tr>
@endif
